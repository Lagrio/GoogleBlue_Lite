...
package me.lagrio.materialqq.googleblue
...
class SubstratumLauncher : Activity() {
    ...
    private val themePiracyCheck by lazy {
        if (me.lagrio.materialqq.googleblue.BuildConfig.ENABLE_APP_BLACKLIST_CHECK) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!me.lagrio.materialqq.googleblue.BuildConfig.SUPPORTS_THIRD_PARTY_SYSTEMS && !supportedSystem) {
            Log.e(tag, "This theme does not support the launching theme system. [HIJACK] ($caller)")
            Toast.makeText(this,
                    String.format(getString(me.lagrio.materialqq.googleblue.R.string.unauthorized_theme_client_hijack), caller),
        ...
        if ((action == substratumIntentData) or (action == getKeysIntent)) {
            ...
            if (organizationsSystem) {
                verified = when {
                    me.lagrio.materialqq.googleblue.BuildConfig.ALLOW_THIRD_PARTY_SUBSTRATUM_BUILDS -> true
        ...
        if (!verified) {
            ...
            Toast.makeText(this, me.lagrio.materialqq.googleblue.R.string.unauthorized_theme_client, Toast.LENGTH_LONG).show()
            finish()
            return
        }
        if (debug) {
    ...
    private fun startAntiPiracyCheck() {
        if (me.lagrio.materialqq.googleblue.BuildConfig.BASE_64_LICENSE_KEY.isEmpty() && debug && !BuildConfig.DEBUG) {
            Log.e(tag, apkSignature)
        }

        if (!themePiracyCheck) {
            piracyChecker {
                if (me.lagrio.materialqq.googleblue.BuildConfig.ENFORCE_GOOGLE_PLAY_INSTALL) {
                    enableInstallerId(InstallerID.GOOGLE_PLAY)
                }
                if (me.lagrio.materialqq.googleblue.BuildConfig.BASE_64_LICENSE_KEY.isNotEmpty()) {
                    enableGooglePlayLicensing(me.lagrio.materialqq.googleblue.BuildConfig.BASE_64_LICENSE_KEY)
                }
                if (me.lagrio.materialqq.googleblue.BuildConfig.APK_SIGNATURE_PRODUCTION.isNotEmpty()) {
                    enableSigningCertificate(me.lagrio.materialqq.googleblue.BuildConfig.APK_SIGNATURE_PRODUCTION)
                }
                callback {
                    allow {
                        ...
                        val themeName = getString(me.lagrio.materialqq.googleblue.R.string.ThemeName)
                        val themeAuthor = getString(me.lagrio.materialqq.googleblue.R.string.ThemeAuthor)
                        ...
                        returnIntent.putExtra("encryption_key", me.lagrio.materialqq.googleblue.BuildConfig.DECRYPTION_KEY)
                        returnIntent.putExtra("iv_encrypt_key", me.lagrio.materialqq.googleblue.BuildConfig.IV_KEY)
                    ...
                    doNotAllow { _, _ ->
                        val parse = String.format(
                                getString(me.lagrio.materialqq.googleblue.R.string.toast_unlicensed),
                                getString(me.lagrio.materialqq.googleblue.R.string.ThemeName))
            ...
            Toast.makeText(this, me.lagrio.materialqq.googleblue.R.string.unauthorized, Toast.LENGTH_LONG).show()
            finish()
        }
    }

    private fun showDialog() {
        val dialog = AlertDialog.Builder(this, me.lagrio.materialqq.googleblue.R.style.DialogStyle)
                .setCancelable(false)
                .setIcon(me.lagrio.materialqq.googleblue.R.drawable.ic_launcher)
                .setTitle(me.lagrio.materialqq.googleblue.R.string.launch_dialog_title)
                .setMessage(me.lagrio.materialqq.googleblue.R.string.launch_dialog_content)
                .setPositiveButton(me.lagrio.materialqq.googleblue.R.string.launch_dialog_positive) { _, _ -> startAntiPiracyCheck() }
        if (getString(me.lagrio.materialqq.googleblue.R.string.launch_dialog_negative).isNotEmpty()) {
            if (getString(me.lagrio.materialqq.googleblue.R.string.launch_dialog_negative_url).isNotEmpty()) {
                dialog.setNegativeButton(me.lagrio.materialqq.googleblue.R.string.launch_dialog_negative) { _, _ ->
                    startActivity(Intent(Intent.ACTION_VIEW,
                            Uri.parse(getString(me.lagrio.materialqq.googleblue.R.string.launch_dialog_negative_url))))
                    finish()
                }
            } else {
